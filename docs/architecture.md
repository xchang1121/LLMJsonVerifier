...
各问题独立分类，返回值来自各自合法候选集。若业务有跨字段约束，例如“未送达”不能同时“已签收”，可以使用明确规则拒绝矛盾结果，或把有关状态合成一个联合候选问题。

`/v1/classify-schema` 将有限 Schema 的可变字段编译为这些独立问题，携带字段路径、父级描述和候选值；常量由程序填入。评分后按编译树重建对象与固定数组，并保留原始 JSON 类型。支持的关键字、结构与预算见 [Schema 接口](schema.md)。
